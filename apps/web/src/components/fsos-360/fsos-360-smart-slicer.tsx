...
export function Fsos360SmartSlicer({ field, labelKey, selected, context, disabled, reason, onChange }: { field: SmartField; labelKey: string; selected: string[]; context: Fsos360QueryInput; disabled?: boolean; reason?: string | null; onChange: (values: string[]) => void }) {
  ...
  const options = useQuery({ queryKey: ["fsos-360", "filter-options", field, search, page, context.currentPeriod, context.comparisonPeriod, context.filters, context.analysisFocus], queryFn: () => fsos360Api.filterOptions({ field, query: search, page, pageSize: 40, context }), enabled: !disabled, placeholderData: (previous) => previous });
