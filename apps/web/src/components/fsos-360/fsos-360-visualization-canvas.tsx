...
export function Fsos360VisualizationCanvas({ data }: { data: Fsos360VisualizationData }) {
  ...
  if (data.kind === "categories") return <ResponsiveContainer width="100%" height={420}><BarChart data={data.items} layout="vertical" margin={{ left: 12 }}><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis type="number" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} /><YAxis type="category" dataKey="label" width={120} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 11 }} /><Tooltip /><Legend /><Bar dataKey="current" name="Current" fill="#2563eb" radius={[4, 4, 4, 4]} /><Bar dataKey="previous" name="Comparison" fill="#0f766e" radius={[4, 4, 4, 4]} /></BarChart></ResponsiveContainer>;
