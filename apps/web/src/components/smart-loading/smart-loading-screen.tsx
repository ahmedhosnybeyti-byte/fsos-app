...
export function SmartLoadingScreen({
  ...
  useEffect(() => {
    if (session?.state === "ready") {
      setSelectedCustomerCodes(new Set(session.routeCustomers.map((customer) => customer.customerCode)));
...
function SmartLoadingPhaseTwo(props: SmartLoadingPhaseTwoProps) {
  ...
  return <>
    <section className="grid gap-3 lg:h-[22rem] lg:grid-cols-[3fr_2fr]">
      <div className="order-1 grid min-h-0 gap-3 lg:col-start-2 lg:row-span-2 lg:grid-rows-[3fr_2fr]">
        ...
        <Card className="h-full min-h-0"><CardHeader className="pb-1 pt-3"><CardTitle className="text-sm">{props.label("smartLoading.sessionSummary", "Session summary")}</CardTitle></CardHeader><CardContent className="grid grid-cols-3 gap-x-2 gap-y-1 pb-3 text-xs"><SessionMetric label={props.label("smartLoading.selectedCustomers", "Selected customers")} value={`${props.selectedCustomerCodes.size}/${props.session.routeCustomers.length}`} /><SessionMetric label={props.label("smartLoading.calendarDays", "Calendar days")} value={days ? String(days) : "—"} /><SessionMetric label={props.label("smartLoading.visitsPerWeek", "Route visits pattern")} value={String(props.visitsPerWeek)} /><SessionMetric label={props.label("smartLoading.orderTotals", "Confirmed products")} value={String(orders.length)} /><SessionMetric label={props.label("smartLoading.totalQuantity", "Total quantity")} value={formatQuantity(orders.reduce((sum, [, quantity]) => sum + quantity, 0), props.locale)} />{props.exceptionalCustomers.length > 0 && <SessionMetric label={props.label("smartLoading.exceptionalCustomers", "Exceptionally added")} value={String(props.exceptionalCustomers.length)} />}</CardContent></Card>
      </div>
      <Card className="order-2 h-full min-h-0 lg:col-start-1 lg:row-span-2"><CardHeader className="pb-2"><CardTitle className="text-base">{props.label("smartLoading.aggregatedConfirmedOrders", "Aggregated confirmed orders")}</CardTitle><CardDescription>{props.label("smartLoading.orderTotals", "Products")}: {orders.length} · {props.label("smartLoading.totalQuantity", "Total quantity")}: {formatQuantity(orders.reduce((sum, [, quantity]) => sum + quantity, 0), props.locale)}</CardDescription></CardHeader><CardContent className="flex min-h-0 flex-1 flex-col space-y-2"><div className="grid gap-2 sm:grid-cols-[1fr_7rem_auto]"><select aria-label={props.label("smartLoading.searchProducts", "Search products")} className="h-9 min-w-0 rounded-md border bg-background px-3" value={props.confirmedProductCode} onChange={(event) => props.onConfirmedProductCodeChange(event.target.value)}><option value="">{props.label("smartLoading.selectProduct", "Select a product")}</option>{props.session.products.map((product) => <option key={product.productCode} value={product.productCode}>{product.productName} ({product.productCode})</option>)}</select><Input className="h-9" type="number" min="1" value={props.confirmedQuantity} onChange={(event) => props.onConfirmedQuantityChange(Math.max(1, Number(event.target.value) || 1))} /><Button className="h-9" type="button" onClick={props.onAddConfirmedOrder} disabled={!props.confirmedProductCode}>{props.label("smartLoading.add", "Add")}</Button></div><div className="min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">{orders.length === 0 ? <p className="py-1 text-sm text-muted-foreground">{props.label("smartLoading.noConfirmedOrders", "No confirmed orders added.")}</p> : orders.map(([code, quantity]) => <div key={code} className="flex items-center justify-between gap-2 rounded border p-2 text-sm"><span className="truncate">{productByCode.get(code)?.productName ?? code}</span><span>{formatQuantity(quantity, props.locale)}</span><Button variant="ghost" size="sm" onClick={() => props.onRemoveConfirmedOrder(code)}>{props.label("smartLoading.remove", "Remove")}</Button></div>)}</div></CardContent></Card>
    </section>
