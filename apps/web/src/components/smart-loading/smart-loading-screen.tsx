...
function ManagementStockAlignmentMetric({ label, value, locale }: { label: string; value: number; locale: "ar" | "en" }) {
  ...
  return (
    <div className="col-span-2 flex items-center justify-between rounded-md bg-background/60 px-2 py-1.5">
      <div>
        <p className="text-[11px] text-muted-foreground">{label}</p>
        <p className="mt-0.5 text-lg font-semibold text-teal-700">{formatQuantity(Math.round(percent), locale)}%</p>
      </div>
      <svg viewBox="0 0 88 50" className="h-10 w-[4.5rem]" role="img" aria-label={`${label}: ${Math.round(percent)}%`}>
