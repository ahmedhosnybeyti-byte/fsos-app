...
function ManagementStockAlignmentMetric({ label, value, locale }: { label: string; value: number; locale: "ar" | "en" }) {
  ...
  return (
    <Card className="min-w-0 border-border/70 bg-background/60 shadow-none">
      <CardContent className="flex items-center justify-between px-2 py-1.5">
      <div className="min-w-0">
        <p className="truncate text-[10px] text-muted-foreground">{label}</p>
        <p className="mt-0.5 text-base font-semibold text-teal-700">{formatQuantity(Math.round(percent), locale)}%</p>
      </div>
      <svg viewBox="0 0 88 50" className="h-8 w-14 shrink-0" role="img" aria-label={`${label}: ${Math.round(percent)}%`}>
        <path d="M 10 44 A 34 34 0 0 1 78 44" fill="none" stroke="currentColor" strokeWidth="7" className="text-muted-foreground/30" strokeLinecap="round" />
        <path d="M 10 44 A 34 34 0 0 1 78 44" fill="none" stroke="currentColor" strokeWidth="7" className="text-teal-600" strokeLinecap="round" strokeDasharray={`${percent * 1.07} 107`} />
        <line x1="44" y1="44" x2="44" y2="17" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" className="text-foreground" transform={`rotate(${needleRotation} 44 44)`} />
        <circle cx="44" cy="44" r="3.5" className="fill-foreground" />
      </svg>
      </CardContent>
    </Card>
  );
...
function SmartLoadingPhaseTwo(props: SmartLoadingPhaseTwoProps) {
  ...
  return <>
    <section className="grid items-stretch gap-3 lg:h-[26rem] lg:grid-cols-[26fr_24fr] lg:grid-rows-[1fr_1fr]" style={{ direction: "ltr" }}>
      ...
      {isManagementRole(props.roleCode) && <ManagementHierarchyFilters locale={props.locale} managementStockAlignmentPercent={props.session.managementStockAlignmentPercent} onManagementScopeChange={props.onManagementScopeChange} />}
...
function ManagementHierarchyFilters({ locale, managementStockAlignmentPercent, onManagementScopeChange }: { locale: "ar" | "en"; managementStockAlignmentPercent: number | null; onManagementScopeChange: (scope: { managerId?: string; supervisorId?: string; salesRepId?: string }) => void }) {
