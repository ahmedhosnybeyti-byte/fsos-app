...
import { calculateVisitSuggestedQuantity, categoryAddedProductCount, getEffectiveAccordionState, groupLostOpportunities, lostOpportunityId, normalizeOpportunityQuantity } from "./lost-opportunity-groups";

const opportunity = (customerCode: string, customerName: string, productCode: string, productName: string, category: string, suggestedQuantity: number) => ({ customerCode, customerName, productCode, productName, category, baselineNetQuantity: suggestedQuantity * 90 / 7, recentNetQuantity: 0, suggestedQuantity });
...
test("groups, searches, and sorts categories, products, and customers by current quantity", () => {
  const groups = groupLostOpportunities(opportunities, {}, "", "Uncategorized", 1);
  ...
  const searchResult = groupLostOpportunities(opportunities, {}, "Ali", "Uncategorized", 1);
...
test("individual drafts rerank immediately without duplicating a product", () => {
  const groups = groupLostOpportunities(opportunities, { [lostOpportunityId("C2", "P1")]: 10 }, "", "Uncategorized", 1);
...
test("ties use stable alphabetic ordering and quantity formatting has one decimal", () => {
  ...
  assert.deepEqual(groupLostOpportunities(tied, {}, "", "Uncategorized", 1).map((group) => group.category), ["Alpha", "Beta"]);
...
test("normalizes drafts to tenths and ignores duplicate customer-product opportunities", () => {
  ...
  const groups = groupLostOpportunities(duplicated, { [lostOpportunityId("C1", "P1")]: 1.26 }, "", "Uncategorized", 1);
  assert.equal(groups[0]!.products.length, 1);
  assert.equal(groups[0]!.products[0]!.customerCount, 1);
  assert.equal(groups[0]!.totalQuantity, 1.26);
  assert.equal(normalizeOpportunityQuantity("-2"), 0);
  assert.equal(normalizeOpportunityQuantity("not-a-number"), 0);
  assert.equal(normalizeOpportunityQuantity(2.345), 2.35);
});

test("search preserves matching category paths and uncategorized groups", () => {
  const groups = groupLostOpportunities([opportunity("C1", "Ali", "P1", "Juice", "", 4), opportunity("C2", "Badr", "P2", "Water", "Drinks", 2)], {}, "Ali", "Uncategorized", 1);
...
test("calculates a per-visit quantity from the 90-day baseline exactly once", () => {
  assert.equal(calculateVisitSuggestedQuantity(90, 1), 7);
  assert.equal(calculateVisitSuggestedQuantity(90, 2), 3.5);
  assert.equal(calculateVisitSuggestedQuantity(90, 6), 7 / 6);
  assert.equal(calculateVisitSuggestedQuantity(30, 1), 7 / 3);
});