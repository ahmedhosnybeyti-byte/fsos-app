...
export interface VisitCopilotProspect {
  ...
  nearbyBestSellers?: Array<{ productCode: string; productName: string; nearbyCustomerCount: number }>;
...
export type { SmartLoadingPriority, SmartLoadingProduct, SmartLoadingManagementVehicleProduct, SmartLoadingRoute, SmartLoadingRouteCustomer, SmartLoadingCustomerSearchResult, SmartLoadingPriorityProduct, SmartLoadingManagementCategoryStockAlignment, SmartLoadingAttention, SmartLoadingReadySession, SmartLoadingStaleProductCustomer, SmartLoadingStaleProductPlan, SmartLoadingManagementStaleRouteProduct, SmartLoadingLostOpportunity, SmartLoadingLostOpportunityReason, SmartLoadingVehicleStockUnavailableSession, SmartLoadingSession, SmartLoadingRecalculateInput, SmartLoadingRecalculatedProduct, SmartLoadingRecalculateResult } from "@field-sales-os/schemas";
...
export type Fsos360VisualizationType = "timeline" | "line" | "bar" | "treemap" | "heat-map" | "coverage-map" | "route-map" | "customer-density";
export interface Fsos360Period { from: string; to: string; }
export interface Fsos360Filters { companyId?: string; regionIds?: string[]; cityValues?: string[]; branchIds?: string[]; managerIds?: string[]; supervisorIds?: string[]; routeIds?: string[]; salesRepIds?: string[]; customerCodes?: string[]; brandValues?: string[]; categoryValues?: string[]; productCodes?: string[]; }
export interface Fsos360QueryInput { currentPeriod: Fsos360Period; comparisonPeriod: Fsos360Period; filters: Fsos360Filters; analysisFocus?: Fsos360AnalysisFocus; visualization?: { preferredType?: "auto" | Fsos360VisualizationType; metric?: "sales" | "collections" | "returns"; groupBy?: "product" | "brand"; }; }
export interface Fsos360Option { value: string; label: string; meta?: Record<string, string>; }
export interface Fsos360RegionCityOption extends Fsos360Option { level: "region" | "city"; parentRegionId?: string; }
export interface Fsos360Kpi { id: "sales" | "collections" | "returns" | "lost-sales" | "orders" | "coverage" | "strike-rate" | "productivity"; currentValue: number | null; previousValue: number | null; changeValue: number | null; changePercentage: number | null; direction: "up" | "down" | "flat" | "not-comparable"; polarity: "favorable" | "unfavorable" | "neutral" | "unknown"; businessMeaning: { code: string; params: Record<string, string | number> } | null; availability: Fsos360Availability; reason: string | null; }
export type Fsos360VisualizationData = { kind: "series"; series: { key: string; label: string; current: number | null; comparison: number | null }[] } | { kind: "categories"; items: { key: string; label: string; current: number; previous: number; change: number }[] } | { kind: "treemap"; groupBy: "product" | "brand"; items: { key: string; label: string; value: number; isOther: boolean }[] } | { kind: "geo-points"; metric: "sales" | "collections" | "returns" | "density" | "coverage"; points: { customerCode: string; customerName: string; routeId: string | null; latitude: number; longitude: number; value: number }[] };
export interface Fsos360Visualization { selectedType: Fsos360VisualizationType; availableTypes: { type: Fsos360VisualizationType; availability: "available" | "unavailable" | "not-applicable"; reason?: string | null }[]; data: Fsos360VisualizationData | null; meta: { totalRows: number; mappedRows?: number; unmappedRows?: number; routeGeometryAvailable?: boolean; generatedAt: string; }; }
export interface Fsos360QueryResult { analysisContext: { currentPeriod: Fsos360Period; comparisonPeriod: Fsos360Period; entityMode: Fsos360AnalysisFocus | "mixed" }; resolvedFilters: Fsos360Filters; removedSelections: Record<string, string[]>; activeAnalysisLevel: Fsos360AnalysisFocus | "mixed"; smallFilterOptions: Record<string, Fsos360Option[]> & { regionCity: Fsos360RegionCityOption[] }; smartSlicerCapabilities: Record<string, { availability: Fsos360Availability; reason: string | null; total?: number }>; executiveInsight: { availability: Fsos360Availability; reason: string | null; items: { code: string; params: Record<string, string | number> }[] }; kpis: Fsos360Kpi[]; performanceComparison: Fsos360Kpi[]; timeline: { granularity: "day" | "week" | "month"; buckets: { position: number; current: number | null; comparison: number | null }[] }; target: { availability: Fsos360Availability; reason: string | null; targetValue?: number; achievementValue?: number; achievementPercentage?: number | null; remainingValue?: number; previousAchievementValue?: number | null }; visualization: Fsos360Visualization; opportunities: { availability: Fsos360Availability; reason: string | null; items: unknown[] }; recommendations: { availability: Fsos360Availability; reason: string | null; items: unknown[] }; capabilities: Record<string, unknown>; generatedAt: string; }
export interface Fsos360FilterOptionsResult { field: "customer" | "product" | "brand" | "category" | "sales-rep"; resolvedFilters: Fsos360Filters; removedSelections: Record<string, string[]>; availability: Fsos360Availability; reason: string | null; options: Fsos360Option[]; page: number; pageSize: number; total: number; hasMore: boolean; }
