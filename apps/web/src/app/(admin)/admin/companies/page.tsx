...
export default function AdminCompaniesPage() {
  ...
  return <div className="relative space-y-6">
    ...
    {temporaryPassword && <div className="glass-card space-y-2 border-primary/50 p-4"><p className="font-medium">Temporary password for {temporaryPassword.email}</p><code className="block break-all rounded bg-secondary p-2">{temporaryPassword.password}</code><p className="text-sm text-muted-foreground">Copy it now. The first Company Admin must change it on sign-in.</p><div className="flex gap-2"><Button size="sm" onClick={() => navigator.clipboard.writeText(temporaryPassword.password).then(() => toast.success("Copied"))}>Copy</Button><Button size="sm" variant="outline" onClick={() => setTemporaryPassword(null)}>Dismiss</Button></div></div>}
