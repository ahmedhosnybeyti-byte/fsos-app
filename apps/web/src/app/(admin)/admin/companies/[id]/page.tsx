...
export default function AdminCompanyDetailsPage() {
  ...
  const lifecycleMutation = useMutation({
    ...
    onSuccess: async (company) => {
      setForm((current) => ({ ...current, status: company.status }));
      queryClient.setQueryData(["admin", "company", params.id], (current: typeof data | undefined) => current ? { ...current, status: company.status } : current);
      await queryClient.invalidateQueries({ queryKey: ["admin", "company", params.id] });
