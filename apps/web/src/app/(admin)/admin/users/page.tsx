...
export default function AdminUsersPage() {
  ...
  const [trialEndUser, setTrialEndUser] = useState<User | null>(null);
  ...
  const trialEndMutation = useMutation({
    mutationFn: () => { if (!trialEndUser || !trialEndValue) throw new Error("Choose a trial end date"); return usersApi.updateTrialEndsAt(trialEndUser.id, new Date(trialEndValue).toISOString()); },
    onSuccess: async () => { setTrialEndUser(null); await invalidate(); toast.success("Trial end updated"); },
    onError: (requestError) => toast.error(requestError instanceof ApiError ? requestError.message : "Could not update trial end"),
  });
