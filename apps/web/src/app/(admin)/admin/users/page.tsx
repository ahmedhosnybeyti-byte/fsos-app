...
export default function AdminUsersPage() {
  ...
  const { data: routeAssignment, refetch: refetchRouteAssignment } = useQuery({ queryKey: ["admin", "user-route-assignment", companyId, routeUser?.id], queryFn: () => usersApi.routeAssignment(routeUser!.id, companyId), enabled: Boolean(routeUser && companyId) });
  ...
  const assignRouteMutation = useMutation({ mutationFn: () => { if (!routeUser || !selectedRouteId) throw new Error("Select a route"); return usersApi.assignRoute(routeUser.id, selectedRouteId, companyId); }, onSuccess: async () => { await Promise.all([invalidate(), refetchRouteAssignment()]); toast.success(routeAssignment?.current ? "Route transferred" : "Route assigned"); }, onError: (error) => toast.error(error instanceof ApiError ? error.message : "Could not update route assignment") });
  const unassignRouteMutation = useMutation({ mutationFn: () => { if (!routeUser) throw new Error("No user selected"); return usersApi.unassignRoute(routeUser.id, companyId); }, onSuccess: async () => { await Promise.all([invalidate(), refetchRouteAssignment()]); toast.success("Route unassigned"); }, onError: (error) => toast.error(error instanceof ApiError ? error.message : "Could not unassign route") });
