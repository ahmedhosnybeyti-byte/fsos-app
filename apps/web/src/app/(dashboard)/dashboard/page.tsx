...
export default function DashboardOverviewPage() {
  ...
  const cards = performance
    ? [[t("performance.sales"), performance.metrics.sales, "currency", CircleDollarSign, "text-emerald-400", false], [t("performance.collections"), performance.metrics.collections, "currency", ClipboardCheck, "text-violet-400", false], [t("performance.invoices"), performance.metrics.invoices, "count", ReceiptText, "text-blue-400", false], [t("performance.customers"), performance.metrics.customers, "count", Users, "text-orange-400", false], [t("performance.skus"), performance.metrics.skus, "count", Package, "text-cyan-400", false], [t("performance.returns"), performance.metrics.returns, "currency", RotateCcw, "text-red-400", true]] as const
    : [];
  if (performanceQuery.isLoading) return <DashboardPerformanceLoading />;
  if (performanceQuery.isError || !performance) return <div className="glass-card p-8 text-center text-sm text-destructive">{t("performance.loadError")}</div>;
  ...
  return (
    <div dir={locale === "ar" ? "rtl" : "ltr"} className="dashboard-light-performance relative isolate space-y-6 bg-slate-50/40 dark:bg-transparent">
      ...
      <div className="glass-hero relative overflow-hidden bg-[linear-gradient(115deg,rgba(255,255,255,.76),rgba(219,234,254,.55),rgba(237,233,254,.58))] p-5 dark:bg-gradient-to-br dark:from-white/[0.09] dark:via-white/[0.045] dark:to-white/[0.01] sm:p-7"><div aria-hidden className="hero-aurora pointer-events-none absolute inset-0" /><div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div><h1 className="flex items-center gap-2 text-2xl font-bold sm:text-3xl"><BarChart3 className="text-primary" />{t("performance.title")}</h1><p className="mt-1 text-sm text-muted-foreground">{t("performance.subtitle")}</p></div><div className="flex flex-wrap items-center gap-3"><Badge variant="outline" className="h-9 px-3 text-sm">{t("performance.sellingDays")}: {formatNumber(performance.sellingDays.elapsed, locale)} / {formatNumber(performance.sellingDays.total, locale)}</Badge><div className="flex rounded-lg border border-border bg-background/30 p-1"><Button size="sm" variant={benchmark === "previous-month" ? "default" : "ghost"} onClick={() => setBenchmark("previous-month")}>{t("performance.previousMonth")}</Button><Button size="sm" variant={benchmark === "previous-quarter-average" ? "default" : "ghost"} onClick={() => setBenchmark("previous-quarter-average")}>{t("performance.previousQuarter")}</Button></div><Button size="icon" variant="outline" aria-label={t("dashboard.refresh")} onClick={() => performanceQuery.refetch()} disabled={performanceQuery.isFetching}><RefreshCw className={performanceQuery.isFetching ? "animate-spin" : ""} /></Button></div></div></div>
...
function formatNumber(value: number, locale: "ar" | "en") { return new Intl.NumberFormat(locale === "ar" ? "ar-SA" : "en-US").format(value); }
function formatPercent(value: number, locale: "ar" | "en", digits = 1) { return new Intl.NumberFormat(locale === "ar" ? "ar-SA" : "en-US", { style: "percent", minimumFractionDigits: digits, maximumFractionDigits: digits }).format(value / 100); }
...
function DashboardGrowthCard({ label, metric, unit, Icon, color, lowerBetter, benchmarkType }: { label: string; metric: DashboardMetric; unit: string; Icon: LucideIcon; color: string; lowerBetter: boolean; benchmarkType: DashboardBenchmark }) {
  ...
  const benchmarkLabel = benchmarkType === "previous-month" ? t("performance.referencePeriod") : t("performance.quarterAverage");
...
function DashboardTargetCard({ target }: { target: DashboardTarget }) {
  const { t, locale } = useTranslation();
  const targetLabelKeys: Record<string, TranslationKey> = { "هدف المبيعات": "performance.targetSales", "هدف التحصيل": "performance.targetCollections", "هدف الوزن": "performance.targetWeight", "هدف العملاء النشطين": "performance.targetActiveCustomers", "هدف الزيارات المنتجة": "performance.targetProductiveCalls", "هدف توزيع الأصناف": "performance.targetSkuDistribution" };
  const targetLabelKey = targetLabelKeys[target.label];
  const targetLabel = targetLabelKey ? t(targetLabelKey) : target.label;
  ...
  return <div className="glass-card border-primary/20 p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{targetLabel}</h3><p className="mt-1 text-sm text-muted-foreground">{t("performance.monthlyTarget")}: {dashboardFormat(target.monthlyTarget, target.unit, locale, t("performance.unavailable"))}</p></div><Badge variant={state.variant}>{state.label}</Badge></div><div className="mt-5 grid grid-cols-3 divide-x divide-x-reverse divide-border"><DashboardStat label={t("performance.actual")} value={dashboardFormat(target.actualMtd, target.unit, locale, t("performance.unavailable"))} /><DashboardStat label={t("performance.targetToDate")} value={dashboardFormat(target.targetMtd, target.unit, locale, t("performance.unavailable"))} /><DashboardStat label={t("performance.difference")} value={target.aheadBehind === null ? t("performance.unavailable") : `${ahead ? "+" : ""}${dashboardFormat(target.aheadBehind, target.unit, locale, t("performance.unavailable"))}`} className={state.text} /></div><div className="mt-5"><div className="mb-2 flex justify-between text-xs text-muted-foreground"><span>{t("performance.achievement")}</span><span>{target.progressPct === null ? t("performance.unavailable") : formatPercent(target.progressPct, locale, 0)}</span></div><div className="h-3 overflow-hidden rounded-full bg-secondary"><div className={`h-full rounded-full ${state.bar}`} style={{ width: `${progress}%` }} /></div></div><div className="mt-5 grid gap-2 sm:grid-cols-3"><DashboardMini label={t("performance.remaining")} value={dashboardFormat(target.remainingMonthlyTarget, target.unit, locale, t("performance.unavailable"))} /><DashboardMini label={t("performance.requiredDaily")} value={dashboardFormat(target.requiredDailyVelocity, target.unit, locale, t("performance.unavailable"))} /><DashboardMini label={t("performance.forecast")} value={dashboardFormat(target.runRateForecast, target.unit, locale, t("performance.unavailable"))} /></div></div>;
}
