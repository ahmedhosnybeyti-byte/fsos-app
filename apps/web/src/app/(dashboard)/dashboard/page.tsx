...
export default function DashboardOverviewPage() {
  ...
  return (
    <div dir="rtl" className="relative space-y-6">
      ...
      <div className="glass-hero relative overflow-hidden p-5 sm:p-7"><div aria-hidden className="hero-aurora pointer-events-none absolute inset-0" /><div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div><h1 className="flex items-center gap-2 text-2xl font-bold sm:text-3xl"><BarChart3 className="text-primary" />{t("performance.title")}</h1><p className="mt-1 text-sm text-muted-foreground">{t("performance.subtitle")}</p></div><div className="flex flex-wrap items-center gap-3"><Badge variant="outline" className="h-9 px-3 text-sm">{t("performance.sellingDays")}: {performance.sellingDays.elapsed} / {performance.sellingDays.total}</Badge><div className="flex rounded-lg border border-border bg-background/30 p-1"><Button size="sm" variant={benchmark === "previous-month" ? "default" : "ghost"} onClick={() => setBenchmark("previous-month")}>{t("performance.previousMonth")}</Button><Button size="sm" variant={benchmark === "previous-quarter-average" ? "default" : "ghost"} onClick={() => setBenchmark("previous-quarter-average")}>{t("performance.previousQuarter")}</Button></div><Button size="icon" variant="outline" onClick={() => performanceQuery.refetch()} disabled={performanceQuery.isFetching}><RefreshCw className={performanceQuery.isFetching ? "animate-spin" : ""} /></Button></div></div></div>
...
function DashboardTargetCard({ target }: { target: DashboardTarget }) {
  ...
  return <div className="glass-card border-primary/20 p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold">{target.label}</h3><p className="mt-1 text-sm text-muted-foreground">{t("performance.monthlyTarget")}: {dashboardFormat(target.monthlyTarget, target.unit)}</p></div><Badge variant={state.variant}>{state.label}</Badge></div><div className="mt-5 grid grid-cols-3 divide-x divide-x-reverse divide-border"><DashboardStat label={t("performance.actual")} value={dashboardFormat(target.actualMtd, target.unit)} /><DashboardStat label={t("performance.targetToDate")} value={dashboardFormat(target.targetMtd, target.unit)} /><DashboardStat label={t("performance.difference")} value={target.aheadBehind === null ? t("performance.unavailable") : `${ahead ? "+" : ""}${dashboardFormat(target.aheadBehind, target.unit)}`} className={state.text} /></div><div className="mt-5"><div className="mb-2 flex justify-between text-xs text-muted-foreground"><span>{t("performance.achievement")}</span><span>{target.progressPct === null ? t("performance.unavailable") : `${target.progressPct.toFixed(0)}%`}</span></div><div className="h-3 overflow-hidden rounded-full bg-secondary"><div className={`h-full rounded-full ${state.bar}`} style={{ width: `${progress}%` }} /></div></div><div className="mt-5 grid gap-2 sm:grid-cols-3"><DashboardMini label={t("performance.remaining")} value={dashboardFormat(target.remainingMonthlyTarget, target.unit)} /><DashboardMini label={t("performance.requiredDaily")} value={dashboardFormat(target.requiredDailyVelocity, target.unit)} /><DashboardMini label={t("performance.forecast")} value={dashboardFormat(target.runRateForecast, target.unit)} /></div></div>;
}
