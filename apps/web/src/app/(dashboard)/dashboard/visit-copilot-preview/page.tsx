...
const PERIODS: { value: VisitCopilotPeriod; labelKey: "visitCopilotPreview.period1m" | "visitCopilotPreview.period3m" | "visitCopilotPreview.period6m" | "visitCopilotPreview.period12m" | "visitCopilotPreview.periodCustom" }[] = [
  { value: "1m", labelKey: "visitCopilotPreview.period1m" }, { value: "3m", labelKey: "visitCopilotPreview.period3m" }, { value: "6m", labelKey: "visitCopilotPreview.period6m" }, { value: "12m", labelKey: "visitCopilotPreview.period12m" }, { value: "custom", labelKey: "visitCopilotPreview.periodCustom" },
];
      ...
      <section className="flex flex-wrap items-end gap-3 rounded-2xl border border-border/70 bg-card p-4">
        <div className="grid gap-1.5">
          ...
          <Select value={period} onValueChange={(value) => { setPeriod(value as VisitCopilotPeriod); resetVisitContext(); }}>
            ...
            <SelectContent>{PERIODS.map((option) => <SelectItem key={option.value} value={option.value}>{t(option.labelKey)}</SelectItem>)}</SelectContent>
          </Select>
        </div>
        {period === "custom" && <>
          <div className="grid gap-1.5"><Label className="text-xs">{t("visitCopilotPreview.from")}</Label><Input type="date" value={from} onChange={(event) => { setFrom(event.target.value); resetVisitContext(); }} className="h-11 w-36" /></div>
          <div className="grid gap-1.5"><Label className="text-xs">{t("visitCopilotPreview.to")}</Label><Input type="date" value={to} onChange={(event) => { setTo(event.target.value); resetVisitContext(); }} className="h-11 w-36" /></div>
        </>}
        <label className="flex h-11 items-center gap-2 text-sm"><Switch checked={vanStock} onCheckedChange={setVanStock} /> {t("visitCopilotPreview.vanStock")}</label>
      </section>

      {!customPeriodReady && <p className="text-sm text-amber-600">{t("visitCopilotPreview.customDateRequired")}</p>}

      {!selectedCustomerCode ? (
        <section className="space-y-3" aria-labelledby="today-plan-title">
          <div className="flex items-end justify-between gap-3">
            <div><p className="text-xs font-medium text-primary">{t("visitCopilotPreview.mode1")}</p><h2 id="today-plan-title" className="text-lg font-semibold">{t("visitCopilotPreview.todayPlan")}</h2></div>
        ...
        <section className="space-y-5" aria-labelledby="visit-mode-title">
          <div className="flex items-center justify-between gap-3">
            <div><p className="text-xs font-medium text-primary">{t("visitCopilotPreview.mode2")}</p><h2 id="visit-mode-title" className="text-lg font-semibold">{t("visitCopilotPreview.visitMode")}</h2></div>
            <Button variant="ghost" className="h-11" onClick={resetVisitContext}><ArrowRight className="h-4 w-4" /> {t("visitCopilotPreview.todayPlan")}</Button>
          </div>
          ...
          {briefingQuery.isLoading ? <div className="space-y-3"><Skeleton className="h-40" /><Skeleton className="h-56" /></div> : briefingQuery.isError ? (
            ...
            <section className="rounded-2xl bg-primary p-5 text-primary-foreground" aria-labelledby="mission-title">
              ...
              <div className="mt-4 flex flex-wrap gap-2"><Badge className="bg-white/15 text-primary-foreground hover:bg-white/15">{demoMeta.mission}</Badge><Badge className="bg-white/15 text-primary-foreground hover:bg-white/15">{t("visitCopilotPreview.priority", { value: Math.round(selectedCustomer?.priorityScore ?? 0) })}</Badge></div>
              <p className="mt-3 text-xs text-primary-foreground/70">{t("visitCopilotPreview.missionNote")}</p>
            </section>
