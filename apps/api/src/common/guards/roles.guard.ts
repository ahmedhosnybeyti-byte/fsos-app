...
export class RolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!requiredRoles.includes(user.roleCode)) {
      await this.userActivity.record({ type: "AUTH_PERMISSION_DENIED", category: "ACCESS", actorUserId: user.userId, subjectUserId: user.userId, actorRole: user.roleCode, companyId: user.companyId, targetType: "Route", targetId: request.originalUrl ? redactSensitiveUrl(request.originalUrl) : request.method, outcome: "DENIED", source: "rbac.roles_guard", requestId: request.requestId, ipAddress: request.ip, userAgent: request.headers?.["user-agent"], metadata: { requiredPermission: requiredRoles.join("|"), method: request.method } });
