...
export class SmartLoadingService {
  ...
  async recalculate(user: AuthenticatedUser, input: SmartLoadingRecalculateInput): Promise<SmartLoadingRecalculateResult> {
    ...
    const ctx = this.rieContext(user); const [customers, products, invoices, items, returns, inventory] = await Promise.all([this.rieFacade.getEntityRecords("Customers", ctx), this.rieFacade.getEntityRecords("Products", ctx), this.rieFacade.getEntityRecords("Invoices", ctx), this.rieFacade.getEntityRecords("Invoice Items", ctx), this.rieFacade.getEntityRecords("Returns", ctx), this.rieFacade.getEntityRecords("Van Inventory", ctx)]);
    if (![customers, products, invoices, items, returns].every((result) => result.available)) throw new BadRequestException("Required RIE data is unavailable.");
    ...
    const selected = new Set(input.customerCodes); const productNames = new Map(products.records.map((row) => [String(row.ProductCode ?? "").trim(), String(row.ProductName ?? row.ProductCode ?? "").trim()]));
    ...
    for (const row of items.records) { const invoice = invoiceMeta.get(String(row.InvoiceNo ?? "").trim()); const code = String(row.ProductCode ?? "").trim(); if (invoice?.date !== null && invoice && selected.has(invoice.customer) && invoice.date >= start && invoice.date <= end && code) net.set(code, (net.get(code) ?? 0) + (toFiniteNumber(row.LineTotal) ?? 0)); }
    for (const row of returns.records) { const date = toEpochMs(row.ReturnDate); const customer = String(row.CustomerCode ?? "").trim(); if (["confirmed", "approved"].includes(String(row.Status ?? "").trim().toLowerCase()) && date !== null && selected.has(customer) && date >= start && date <= end) { const code = String(row.ProductCode ?? "").trim(); if (code) net.set(code, (net.get(code) ?? 0) - (toFiniteNumber(row.TotalAmount) ?? 0)); } }
