...
export class LostOpportunityService {
  ...
  async detect(input: LostOpportunityRequest): Promise<LostOpportunityResult> {
    ...
    try {
      ...
      for (const invoice of invoices.records) { const invoiceNo = key(invoice.InvoiceNo); const customerCode = key(invoice.CustomerCode); const ms = dateValue(invoice.InvoiceDate); if (key(invoice.InvoiceStatus).toLowerCase() === "confirmed" && invoiceNo && customerCodes.has(customerCode) && Number.isFinite(ms)) invoicesByNo.set(invoiceNo, { customerCode, date: isoDay(new Date(ms)) }); }
