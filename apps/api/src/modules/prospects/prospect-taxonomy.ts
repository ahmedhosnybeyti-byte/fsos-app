...
const CHANNEL_RULES: readonly ChannelRule[] = [
  ...
  { aliases: ["wholesale", "cash van", "cashvan"], segment: "RETAIL_FMCG", category: "wholesale" },
  { aliases: ["traditional trade", "traditional", "tt", "retail", "fmcg", "grocery", "convenience"], segment: "RETAIL_FMCG", category: "traditional" },
];
...
export function taxonomyForCanonicalChannel(channel: string | null): { segment: ProspectMarketSegment; category: DiscoveryCategory; matched: boolean } | null {
  ...
  const rule = CHANNEL_RULES.find((candidate) => candidate.aliases.includes(normalized));
