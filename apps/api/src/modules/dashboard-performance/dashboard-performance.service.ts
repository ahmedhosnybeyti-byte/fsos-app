...
export class DashboardPerformanceService {
  ...
  async get(user: AuthenticatedUser, benchmark: DashboardBenchmark, routeIds?: string[]) {
    const now = new Date(); const start = monthStart(now); const next = monthStart(now, 1); const ctx = this.ctx(user); const selectedRoutes = routeIds?.length ? new Set(routeIds) : null;
