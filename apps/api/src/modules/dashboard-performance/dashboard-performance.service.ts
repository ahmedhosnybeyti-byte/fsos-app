...
export class DashboardPerformanceService {
  ...
  async get(user: AuthenticatedUser, benchmark: DashboardBenchmark) {
    ...
    const actual: Record<string, number | null> = { sales: cur.sales, collections: cur.collections, weight: productWeight.size ? cur.weight : null, customers: cur.customers.size, productiveCalls: cur.productiveCalls, skus: cur.skus.size };
    ...
    return { periodMonth: key(start.getTime()).slice(0, 7), benchmark, sellingDays: { elapsed, total, remaining: Math.max(total - elapsed, 0), available: total > 0 }, metrics: { sales: metric(cur.sales, avg((a) => a.sales), true), collections: metric(cur.collections, avg((a) => a.collections)), invoices: metric(cur.invoices.size, avg((a) => a.invoices.size)), customers: metric(cur.customers.size, avg((a) => a.customers.size)), skus: metric(cur.skus.size, avg((a) => a.skus.size)), returns: metric(cur.returns, avg((a) => a.returns)) }, targets: targetCards, warnings };
  }
  ...
  private async previousQuarterDates(companyId: string, now: Date, count: number) { const qStartMonth = Math.floor(now.getUTCMonth() / 3) * 3; const priorQuarterStart = new Date(Date.UTC(now.getUTCFullYear(), qStartMonth - 3, 1)); return Promise.all([0, 1, 2].map((i) => this.firstSellingDates(companyId, new Date(Date.UTC(priorQuarterStart.getUTCFullYear(), priorQuarterStart.getUTCMonth() + i, 1)), count))); }
}
