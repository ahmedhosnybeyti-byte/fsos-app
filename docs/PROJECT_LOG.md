...
**تصحيح ثانٍ (نفس اليوم) — فشل build تاني مختلف**: بعد الـpush التاني، فشل الـbuild بخطأ TS مختلف عند `chart-engine.tsx:390` — دالة `label` المخصّصة لدائرة الـPie كانت بتاخد `textAnchor: string` بس recharts' `PieLabelRenderProps` عندها نوع union صارم لـ`textAnchor` (`"middle" | "inherit" | "end" | "start" | undefined`)، فمش متوافق مع `string` العام. الإصلاح: استبدلت توقيع `entry` بـ`any` مع تعليق `eslint-disable` موثّق (بنفس نمط الـworkaround الموجود مسبقًا في نفس الملف لـ`Treemap`'s `onClick` — النوع الحقيقي لخصائص عقدة recharts في وقت التشغيل مش معبَّر عنه بدقة في التعريفات المتاحة). لم يُلمس أي منطق آخر.

**تعديلان إضافيان بعد المعاينة الحية (نفس اليوم)**: بعد نجاح الـbuild وشوف العميل الشاشة فعليًا، طلب تعديلين: (1) تسميات المحور لسه ملزّقة بخط الصفر — تمت إضافة `tickMargin={12}` لكل عناصر `<XAxis>` الأربعة المائلة (Column/Line/Area/Pareto) وزيادة `height` من 70 لـ80، عشان مسافة حقيقية بين آخر خط شبكة والتسمية. (2) تأثير الكريستال كان بيغسل اللون الأساسي بلمعة بيضاء عريضة بدل ما يديله لمعة فوقه — بعد ما بعت العميل صورة مرجعية (أعمدة كريستال ملوّنة بوضوح مع انعكاس ضوئي رفيع)، أعدت بناء تدرج `CrystalGradientDefs`: اللون الدلالي (أخضر/أصفر/برتقالي/أحمر) بقى هو المسيطر من أول لآخر العمود، مع شريط أبيض رفيع (8%–16% من الارتفاع) كطبقة انعكاس فوق اللون بدل ما يستبدله، فالعمود يفضل واضح إنه بلون أدائه الحقيقي وعليه لمعة، مش أبيض شبه شفاف.

**تراجع كامل عن تأثير الكريستال (نفس اليوم) — الأعمدة ظهرت بلا لون خالص على الإنتاج**: بعد نشر تعديل التدرج المُصحَّح، العميل أبلغ إن الأعمدة على البيئة الحية بقت بلا أي لون دلالي خالص (رمادي/شفاف بالكامل)، رغم إن نفس التدرج كان شغّال منطقيًا في الكود. اتعمل تشخيص (بحث عن سلوك recharts مع `<defs>` مُغلَّفة في مكوّن دالة مخصّص) بدون التوصل لسبب قاطع بعد — كان المفروض نكمل بفحص DOM/Elements الفعلي في المتصفح لتأكيد القيمة الحقيقية لـ`fill`/وجود `<linearGradient>` في الصفحة، لكن العميل قاطع الفحص وطلب صراحة التراجع الكامل عن الكريستال والرجوع لدرجات الألوان الأصلية الثابتة (`SEMANTIC_COLORS` الخام عبر `barColors[i]`) بدون أي تعديل.

**التنفيذ**: أُزيلت بالكامل من `chart-engine.tsx`: دالة `CrystalGradientDefs()`، دالة `crystalFill()`، دالة `crystalFillForHex()`، الثابت `CRYSTAL_GRADIENT_IDS`، الخريطة العكسية `HEX_TO_STATUS`، وكل استدعاء لـ`<CrystalGradientDefs />` من داخل كل أنواع الرسم (Column/Bar/Stacked/Scatter/Pareto). كل `fill={crystalFillForHex(barColors[i])}` رجع لـ`fill={barColors[i]}` المباشر (اللون الدلالي الخام بدون أي تدرج). حدود اللمعة البيضاء الشفافة (`stroke="rgba(255,255,255,0.35)"`) اللي كانت مضافة كجزء من نفس التأثير اتشالت من كل الأعمدة، ورجع الأصلي: `stroke="none"` لغير المحدَّد، `SEMANTIC_COLORS.neutral` بعرض 2 للعمود المحدَّد (highlighted) — تمامًا زي السلوك قبل أي تعديل كريستال. **تسمية المحور (tickMargin/height) اتسابت زي ما هي** — ده إصلاح منفصل خالص لمشكلة وضوح فعلية، مش جزء من طلب التراجع.

**التحقق**: توازن الأقواس (239/239 و212/212 — سليم)، تأكيد صفر مراجع متبقية لأي من الأسماء الأربعة المحذوفة، تأكيد `SEMANTIC_COLORS` لسه مستوردة ومستخدمة (5 مرات، للـstroke المحدَّد وخط باريتو التراكمي فقط). **الدرس المستفاد**: لما تعديل بصري بسيط (تلوين) يحوّل لسبب غموض فعلي في الإنتاج بعد عدة محاولات إصلاح بدون تشخيص حاسم، الأولوية ترجع للاستقرار المعروف (الشكل الأصلي الشغّال) بدل الاستمرار في التخمين — قرار العميل صحيح ومباشر.
